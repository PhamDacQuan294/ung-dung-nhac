...
if(buttonsDelete.length > 0) {
  ...
  buttonsDelete.forEach(button => {
    button.addEventListener("click", () => {
      const isConfirm = confirm("Bạn có chắc muốn xoá ca sỹ này không?");
...
// Form create topic
const formCreateSinger = document.querySelector("#form-create-singer");

if(formCreateSinger) {
  formCreateSinger.addEventListener("submit", (e) => {
    e.preventDefault();

    const redirectUrl = window.location.pathname + window.location.search;
    formCreateSinger.action += `?redirect=${encodeURIComponent(redirectUrl)}`;

    formCreateSinger.submit();
  })
}
